...
const Modal = ({ showModal, title, typeModal, text, statusModal, iconModal, hideModal } : IModal) => {
    ...
    return(
        <>
            ...
            <div className="z-index-999 bg-black-solid position-fixed center-position-absolute border-solid-light-purple-2 box-shadow-light-purple-0-0-20 border-radius-3 w-100 h-100  top-0 size-modal max-h-300 animation-show-modal">
                <div className="position-relative w-100 h-100 d-flex justify-content-center align-items-center flex-column gap-3 p-3">
                    
                    {
                        typeModal !== 'loading' ? (
                            <span onClick={hideModal} className="material-symbols-outlined position-absolute top-0 right-0 m-3 cursor-pointer color-white">
                                close
                            </span>
                        ) : null
                    }
